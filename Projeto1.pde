...
//Ceu
  ...
  fill(0);
  noStroke();
  float eyeX = xPessoa + 15;  
  float eyeY = yPessoa - 5;  
  float eyeSize = 10;        
  float mouseXOffset = mouseX - eyeX;
  float mouseYOffset = mouseY - eyeY;
  float angle = atan2(mouseYOffset, mouseXOffset);
  float eyeXOffset = cos(angle) * eyeSize * 0.5;
  float eyeYOffset = sin(angle) * eyeSize * 0.5;


  ellipse(eyeX + eyeXOffset, eyeY + eyeYOffset, eyeSize, eyeSize);
  ellipse(eyeX - eyeXOffset, eyeY - eyeYOffset, eyeSize, eyeSize);
}
...
void keyPressed() {
  ...
  if (key == 'g') {
    girarEstrela = true; // Inicia a rotação da estrela
  } else if (key == 'p') {
    girarEstrela = false; // Para a rotação da estrela
  }
  
  if (keyCode == LEFT) {
    xPessoa -= 10; // Move o personagem para a esquerda
  } else if (keyCode == RIGHT) {
    xPessoa += 10; // Move o personagem para a direita
  }
}
